    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
        ...
        <div class="lg:col-span-1 flex flex-col space-y-4">
            ...
            <div class="bg-white p-4 shadow-sm sm:rounded-lg">
                <label for="search" class="block text-sm font-medium text-gray-700 mb-1">Find Student</label>
                <div class="relative">
                ...
                <p class="text-xs text-gray-500 mt-2">Try "Juan" or "Maria"</p>
            </div>

            <!-- Student Card -->
            @if($student)
                <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden flex-1 flex flex-col">
                    <div class="p-6 flex flex-col items-center border-b border-gray-200 bg-gray-50">
                        <img class="h-24 w-24 rounded-full border-4 border-white shadow-md mb-4"
                             src="https://ui-avatars.com/api/?{{ $student['avatar_params'] }}&size=128"
                             alt="{{ $student['name'] }}">
                        <h2 class="text-xl font-bold text-gray-900 text-center">{{ $student['name'] }}</h2>
                        <p class="text-sm text-gray-500">{{ $student['lrn'] }}</p>
                    ...
                    <div class="p-6 flex-1 flex flex-col justify-center items-center text-center">
                        <p class="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Outstanding Balance</p>
                        <div class="p-4 rounded-lg bg-gray-50 border w-full {{ $student['balance'] > 0 ? 'border-red-200' : 'border-green-200' }}">
                            <span class="block text-3xl font-extrabold {{ $student['balance'] > 0 ? 'text-red-600' : 'text-green-600' }}">
            ...
            @elseif(!empty($search))
                <div class="bg-white p-8 shadow-sm sm:rounded-lg flex items-center justify-center text-gray-500 flex-1">
            ...
            @else
                <div class="bg-white p-8 shadow-sm sm:rounded-lg flex items-center justify-center text-gray-400 flex-1">
                    <div class="text-center">
        ...
        <div class="lg:col-span-2 bg-white overflow-hidden shadow-sm sm:rounded-lg flex flex-col h-full border border-gray-200">

            <!-- Header -->
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                <h3 class="text-lg font-bold text-gray-800 flex items-center">
                    <i class='bx bx-cart-alt mr-2 text-indigo-600'></i>
                    Current Transaction
                </h3>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($cart as $index => $item)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right font-mono">₱ {{ number_format($item['amount'], 2) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button wire:click="removeItem({{ $index }})" class="text-red-600 hover:text-red-900 transition-colors">
            ...
            <div class="bg-gray-50 px-6 py-4 border-t border-gray-200">
                <div class="flex justify-between items-end mb-4">
                    <span class="text-sm font-medium text-gray-500 uppercase">Total Amount Due</span>
                    <span class="text-3xl font-bold text-gray-900">₱ {{ number_format($this->total, 2) }}</span>
                </div>
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                ...
                                <div class="mt-4 space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <select wire:model.live="newItemType" id="itemType" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            ...
                                            <input type="number"
                                                   ...
                                                   id="amount"
                                                   class="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-7 pr-12 sm:text-sm border-gray-300 rounded-md {{ $isStoreItem ? 'bg-gray-100 cursor-not-allowed' : '' }}"
                                                   placeholder="Enter Amount"
                                                   {{ $isStoreItem ? 'readonly' : '' }}>
                                        </div>
                                        @if($isStoreItem)
                                            <p class="mt-1 text-xs text-gray-500">Fixed price for this item.</p>
                                        @else
                                            <p class="mt-1 text-xs text-gray-500">Enter payment amount.</p>
                                        @endif
