<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-800">School Fee Setup</h1>
        ...
        @foreach($gradeLevels as $grade)
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col h-full">
                ...
                <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-lg">
                    <h2 class="text-lg font-semibold text-gray-800">Grade {{ $grade }} Fees</h2>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($gradeFees as $fee)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-3 whitespace-nowrap text-sm text-gray-900">{{ $fee['description'] }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap text-sm text-gray-600">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $fee['type'] === 'Tuition' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800' }}">
                                            {{ $fee['type'] }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-3 whitespace-nowrap text-right text-sm font-medium space-x-1">
                                        <button wire:click="editFee({{ $fee['id'] }})" class="text-indigo-600 hover:text-indigo-900 p-1 hover:bg-indigo-50 rounded" title="Edit">
                                            <i class='bx bx-edit'></i>
                                        </button>
                                        <button wire:click="deleteFee({{ $fee['id'] }})" class="text-red-600 hover:text-red-900 p-1 hover:bg-red-50 rounded" title="Delete" onclick="confirm('Are you sure?') || event.stopImmediatePropagation()">
                ...
                <div class="bg-gray-50 px-6 py-3 border-t border-gray-200 rounded-b-lg">
                    <div class="flex justify-between items-center">
                        <span class="text-sm font-medium text-gray-600">Total Assessment for Grade {{ $grade }}:</span>
                        <span class="text-lg font-bold text-gray-900 font-mono">₱{{ number_format($totalAmount, 2) }}</span>
                    </div>
                </div>
            </div>
        @endforeach
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full {{ $isEditing ? 'bg-indigo-100' : 'bg-green-100' }} sm:mx-0 sm:h-10 sm:w-10">
                            <i class='bx {{ $isEditing ? 'bx-edit' : 'bx-plus' }} {{ $isEditing ? 'text-indigo-600' : 'text-green-600' }} text-xl'></i>
                        </div>
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-4 space-y-4">
                                <!-- Description -->
                                <div>
                                    <label for="description" class="block text-sm font-medium text-gray-700">Fee Description</label>
                                    <input type="text" wire:model="editingDescription" id="description" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="e.g. Energy Fee">
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" wire:model="editingAmount" id="amount" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="0.00">
                                ...
                                <div>
                                    ...
                                    <select wire:model="editingType" id="type" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
